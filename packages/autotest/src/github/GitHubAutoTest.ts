...
export class GitHubAutoTest extends AutoTest implements IGitHubTestManager {
	...
	public async handleCommentEvent(info: CommitTarget): Promise<boolean> {
		...
		if (preconditionsMet === false) {
			Log.info("GitHubAutoTest::handleCommentEvent( " + info.personId + " ) - skipped; preconditions not met");
			return false;
		}
		...
		if (typeof pushEvent?.ref === "string") {
			...
			Log.trace(
				"GitHubAutoTest::handleCommentEvent( " +
					info.personId +
					" ) - processing; deliv: " +
					info.delivId +
					"; repo: " +
	...
	protected async postToGitHub(target: CommitTarget, message: IGitHubMessage): Promise<boolean> {
		if (typeof target.flags !== "undefined" && target.flags.indexOf("#silent") >= 0) {
			...
			Log.info(
				"GitHubAutoTest::postToGitHub(..) - posting" +
					"; deliv: " +
					target.delivId +
					"; repo: " +
					target.repoId +
	...
	protected async processCommentExists(info: CommitTarget, res: AutoTestResultTransport): Promise<void> {
		...
		Log.info(
			"GitHubAutoTest::processCommentExists(..) - handling request for: " +
				info.personId +
				"; deliv: " +
				info.delivId +
				"; repo: " +
		...
		if (res.output.postbackOnComplete === false) {
			...
			if (previousFeedback === null) {
				...
				if (res.input.target.personId !== info.personId) {
					...
					Log.info(
						"GitHubAutoTest::processCommentExists(..) - updating target not needed for: " +
							info.personId +
							"; deliv: " +
							info.delivId +
							"; repo: " +
				...
				Log.info(
					"GitHubAutoTest::processCommentExists(..) - result already exists and paid for for: " +
						info.personId +
						"; SHA: " +
						Util.shaHuman(info.commitSHA)
				);
			...
			Log.info(
				"GitHubAutoTest::processCommentExists(..) - result already exists; feedback request skipped for: " +
					info.personId +
					"; SHA: " +
					Util.shaHuman(info.commitSHA)
			);
	...
	protected async processCommentNew(info: CommitTarget): Promise<void> {
		Log.info(
			"GitHubAutoTest::processCommentNew(..) - handling request for user: " +
				info.personId +
				"; deliv: " +
				info.delivId +
				"; repo: " +
		...
		if (containerConfig !== null) {
			...
			if (onQueue === true) {
				...
				if (pe === null) {
					...
					Log.warn(
						"GitHubAutoTest::processCommentNew(..) - push event was not present; adding now. URL: " +
							info.commitURL +
							"; for: " +
							info.personId +
							"; SHA: " +
	...
	protected async handleCommentStudent(target: CommitTarget, res: AutoTestResultTransport): Promise<void> {
		Log.info(
			"GitHubAutoTest::handleCommentStudent(..) - handling student request for: " +
				target.personId +
				"; deliv: " +
				target.delivId +
				"; repo: " +
		...
		if (shouldCharge === true && previousRequest === null && feedbackDelay !== null) {
			Log.info(
				"GitHubAutoTest::handleCommentStudent(..) - too early for: " +
					target.personId +
					"; must wait: " +
					feedbackDelay +
					"; SHA: " +
			...
			Log.info(
				"GitHubAutoTest::handleCommentStudent(..) - feedback previously given for: " +
					target.personId +
					"; deliv: " +
					target.delivId +
					"; SHA: " +
	...
	protected async processExecution(data: AutoTestResult): Promise<void> {
		try {
			...
			const feedbackDelay: string | null = await this.requestFeedbackDelay(
				delivId,
				personId,
				data.input.target.timestamp,
				data.input.target.flags
			);
			...
			Log.info(
				"GitHubAutoTest::processExecution() - " +
					...
					data.repoId +
					"; SHA: " +
					Util.shaHuman(data.commitSHA) +
					"; postback: " +
					data.output.postbackOnComplete +
					"; requested: " +
					feedbackRequested +
					"; delay: " +
					feedbackDelay +
			...
			if (data.output.postbackOnComplete === true) {
				...
				if (futureTarget === true) {
					Log.info(
						`GitHubAutoTest::processExecution() - postback true for futureTarget` +
							`removing: ${data.input.target.personId} from scheduleQueue.`
					);
				}
				// do this first, does not count against quota
				Log.trace(
					"GitHubAutoTest::processExecution() - postback: true; deliv: " +
						delivId +
						"; repo: " +
						data.repoId +
						"; SHA: " +
				...
				feedbackRequested === true && // person requested feedback
				...
				if (feedbackDelay !== null) {
					Log.info(
						"GitHubAutoTest::processExecution() - commit no longer eligible for receiving feedback: " +
							"deliv: " +
							data.delivId +
							"; repo: " +
							data.repoId +
					...
					Log.info(
						"GitHubAutoTest::processExecution() - commit not requested - no feedback given; deliv: " +
							data.delivId +
							"; repo: " +
							data.repoId +
							"; SHA: " +
	...
	protected async requestNextTimeslot(delivId: string, userName: string): Promise<number | null> {
		...
		if (record) {
			...
			Log.info(
				"GitHubAutoTest::requestNextTimeslot( " +
					delivId +
					", " +
					userName +
					" ) - delay: " +
					...
					"; next: " +
					new Date(nextTimeslot).toLocaleTimeString()
			);
			return nextTimeslot;
		} else {
			return null;
		...
		if (auth !== null && (auth.isAdmin === true || auth.isStaff === true)) {
			...
			if (typeof info.flags !== "undefined") {
				...
				if (info.flags.indexOf("#force") >= 0) {
					Log.info("GitHubAutoTest::checkCommentPreconditions( " + info.personId + " ) - ignored, student use of #force");
				...
				if (info.flags.indexOf("#dev") >= 0) {
					Log.info("GitHubAutoTest::checkCommentPreconditions( " + info.personId + " ) - ignored, student use of #dev");
				...
				if (info.flags.indexOf("#silent") >= 0) {
					Log.warn("GitHubAutoTest::checkCommentPreconditions( " + info.personId + " ) - ignored, student use of #silent");
	...
	private async shouldCharge(info: CommitTarget, isStaff: AutoTestAuthTransport, res: AutoTestResultTransport): Promise<boolean> {
		...
		if (typeof isStaff !== "undefined" && isStaff !== null && (isStaff.isAdmin === true || isStaff.isStaff === true)) {
		...
		try {
			...
			if (feedbackDelay !== null) {
				...
				if (feedbackDelay.accepted === true) {
					...
					Log.info(
						"GitHubAutoTest::requestFeedbackDelay( " +
							userName +
							" ) - custom done; can NOT request feedback; message: " +
							feedbackDelay.message +
							"; fullMessage: " +
							feedbackDelay?.fullMessage
					);
					let msg = "";
	...
	private async saveFeedbackGiven(delivId: string, userName: string, timestamp: number, commitURL: string, kind: string): Promise<void> {
		try {
			Log.info(
				"GitHubAutoTest::saveFeedbackGiven(..) - feedback request saved" +
					"; person: " +
					userName +
					"; deliv: " +
					delivId +
					"; repo: " +
					GitHubUtil.commitURLtoRepoName(commitURL)
			);
